...
export class Si58Device1759219145956 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "studio_cdn" ("ID" SERIAL NOT NULL, "TABLE_ID" character varying(255) NOT NULL, "CDN" jsonb NOT NULL, "CREATED_AT" TIMESTAMP NOT NULL DEFAULT now(), "UPDATED_AT" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "pk_studio_cdn_id" PRIMARY KEY ("ID")); COMMENT ON COLUMN "studio_cdn"."ID" IS 'Auto increase number'; COMMENT ON COLUMN "studio_cdn"."TABLE_ID" IS 'Unique Table Name'; COMMENT ON COLUMN "studio_cdn"."CDN" IS 'CDN Destination'; COMMENT ON COLUMN "studio_cdn"."CREATED_AT" IS 'When the player session was created'; COMMENT ON COLUMN "studio_cdn"."UPDATED_AT" IS 'When the player session was updated'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "studio_device" ("ID" SERIAL NOT NULL, "DEVICE_ID" character varying(255) NOT NULL, "TABLE_ID" character varying(255) NOT NULL, "CREATED_AT" TIMESTAMP NOT NULL DEFAULT now(), "UPDATED_AT" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "pk_studio_device_id" PRIMARY KEY ("ID")); COMMENT ON COLUMN "studio_device"."ID" IS 'Auto increase number'; COMMENT ON COLUMN "studio_device"."DEVICE_ID" IS 'Unique device Name'; COMMENT ON COLUMN "studio_device"."TABLE_ID" IS 'Table Id'; COMMENT ON COLUMN "studio_device"."CREATED_AT" IS 'When the player session was created'; COMMENT ON COLUMN "studio_device"."UPDATED_AT" IS 'When the player session was updated'`,
    );
    await queryRunner.query(
